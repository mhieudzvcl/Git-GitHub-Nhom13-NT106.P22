...
namespace Login_or_Signup
{
    public partial class Lobby : Form
    {
        ...
        private async void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            VolumeProgressBar.Visible = true;
        ...
        private void guna2GradientButton2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Type = "create_room",
                    RoomName = guna2TextBox2.Text.Trim(),
                    RoomPassword = guna2TextBox4.Text.Trim(),
                    HostUsername = txtNameInApp.Text.Trim()
                };
                ...
                // Nhận phản hồi duy nhất
                byte[] buffer = new byte[4096];
                int bytesRead = stream.Read(buffer, 0, buffer.Length);

                if (bytesRead == 0)
                {
                    MessageBox.Show("Server không gửi phản hồi!");
                    client.Close();
                    return;
                }

                string responseJson = Encoding.UTF8.GetString(buffer, 0, bytesRead);
               // MessageBox.Show("Phản hồi server:\n" + responseJson);

                CreateRoomResponse result;
                try
                {
                    result = JsonSerializer.Deserialize<CreateRoomResponse>(responseJson);
                }
                catch (Exception ex)
                {
                ...
                {
                    MessageBox.Show("Phòng đã được tạo thành công!");
                    this.Hide();

                    // Load avatar host
                    System.Drawing.Image hostImage;
                    if (!string.IsNullOrEmpty(result.avthostname) && System.IO.File.Exists(result.avthostname))
                    {
                        hostImage = System.Drawing.Image.FromFile(result.avthostname);
                    }
                    else
                    {
                        hostImage = Properties.Resources.anhdaidienmacdinh2;
                    }

                    Room room = new Room(serverIP, result.roomID, result.roomName, result.hostname, hostImage, client, result.hostname);
                    room.AddHostToSlot1(result.hostname, result.avthostname);
        ...
        {
            if (string.IsNullOrWhiteSpace(txtRoomID.Text) || string.IsNullOrWhiteSpace(txtPasswordRoom.Text))
            {
                MessageBox.Show("Vui lòng nhập đầy đủ Room ID và mật khẩu");
                return;
            }

            try
            {
                var client = new TcpClient(serverIP, 9000);
                NetworkStream stream = client.GetStream();
                ...
                var request = new
                {
                    ...
                    RoomPassword = txtPasswordRoom.Text.Trim(),
                    NameInApp = txtNameInApp.Text.Trim(),
                };
                ...
                // Đọc phản hồi thứ 1: join_success
                byte[] buffer = new byte[4096];
                int bytesRead = stream.Read(buffer, 0, buffer.Length);
                string response1 = Encoding.UTF8.GetString(buffer, 0, bytesRead);
                var statusObj = JsonSerializer.Deserialize<StatusResponse>(response1);
                
                if (statusObj.status == "join_success")
                {
                    MessageBox.Show("Tham gia phòng thành công!");

                    // Đọc tiếp phản hồi thứ 2: room_member_list
                    bytesRead = stream.Read(buffer, 0, buffer.Length);
                    string response2 = Encoding.UTF8.GetString(buffer, 0, bytesRead);

                    var roomData = JsonSerializer.Deserialize<ServerBroadcast>(response2);
                    MessageBox.Show("Phản hồi server:\n" + response2);

                    // Load avatar host
                    System.Drawing.Image hostAvt;
                    if (!string.IsNullOrEmpty(roomData.avthostname) && System.IO.File.Exists(roomData.avthostname))
                    {
                        hostAvt = System.Drawing.Image.FromFile(roomData.avthostname);
                    }
                    else
                    {
                        hostAvt = Properties.Resources.anhdaidienmacdinh2;
                    }
                    
                     Room room = new Room(serverIP, txtRoomID.Text.Trim(), roomData.roomName, roomData.hostname, hostAvt, client, roomData.NameInApp);
                   // Room room = new Room(serverIP, txtRoomID.Text.Trim(), roomData.roomName, client);

                    room.AddHostToSlot1(roomData.hostname, roomData.avthostname);
                    foreach (var member in roomData.member)
                    {
                        room.AddMemberToUISlot(member.name, member.avatar, member.slot);
                    }
                ...
                {
                    MessageBox.Show("Tham gia phòng thất bại! Kiểm tra lại RoomID hoặc mật khẩu.");
                }
            }
            catch (Exception ex)
            {
